...
echo $line_number
LINE=$(sed -n "${line_number}p" ./parameters/centroid_understanding_z_2.0.txt)
...
conda activate HeST
./optimization_runs.py --file_path="${INPUT_FILE[0]}" --num_qps=2000000  --refl_prob=0.15 --setup=7 --pos="${INPUT_FILE[1]}"
