...
def Evaporation_He_Surface(particle,detector,where_is_Particle):
    ...
    sin_critical_angle = np.sqrt( 2*(energy-0.00062)*(4.002603254*1e9) )/momentum
    ...
    if theta_I > critical_angle_deg:
        ...
        sin_theta_R = (momentum)*np.sin(np.deg2rad(theta_I))/np.sqrt( 2*(energy-0.00062)*(4.002603254*1e9) )
        ...
        if direction[1] == 0.:
            ...
            tan_degree = np.arctan(np.abs(direction[0]/direction[1]) )
